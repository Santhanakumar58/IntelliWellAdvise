...
from .forms import GPMultiRateDrawdownTestForm, GPMultiRateDrawdownTestUploadForm
from .models import GPMultiRateDrawdowntest, filepath
import pandas as pd
import numpy as np
...
def list_gpmultirate_test_data(request):
    selectedwell = SelectedGasProducer.objects.first()   
    drawdown_test_datas = GPMultiRateDrawdowntest.objects.filter(gpwellid =selectedwell.wellid).all()   
    return render (request, 'gpmultiratedrawdowntestanalysis/gpmultirate_test_data.html', {'drawdown_test_datas': drawdown_test_datas })   
 
def create_gpmultirate_test_data(request):    
    drawdown = GPMultiRateDrawdowntest()
    selectedwell = SelectedGasProducer.objects.first()  
    drawdown.fgid = selectedwell.fgid
    drawdown.wellid = selectedwell.wellid   
    form = GPMultiRateDrawdownTestForm(request.POST or None, instance=drawdown)
    if request.method =="POST": 
        drawdown.gpfgid = selectedwell.fgid
        drawdown.gpwellid = selectedwell.wellid  
        form = GPMultiRateDrawdownTestForm(request.POST, request.FILES, instance=drawdown)                    
        if form.is_valid(): 
            form.save()  
            file_Name = form.instance
            return redirect ('gpmultiratedrawdowntestanalysis:list_gpmultirate_test_data') 
    return render (request, 'gpmultiratedrawdowntestanalysis/gpmultirate_test_data_form.html', {'form': form})

def update_gpmultirate_test_data(request, id): 
   drawdown= GPMultiRateDrawdowntest.objects.get(id=id)  
   form = GPMultiRateDrawdownTestForm(request.POST or None, instance=drawdown) 
   if request.method =="POST":       
        form = GPMultiRateDrawdownTestForm(request.POST, request.FILES, instance=drawdown)        
        if form.is_valid():
            form.save()           
            return redirect ('gpmultiratedrawdowntestanalysis:list_gpmultirate_test_data')
   return render (request, 'gpmultiratedrawdowntestanalysis/gpmultirate_test_data_form.html', {'form': form, 'drawdown':drawdown})

def delete_gpmultirate_test_data(request, id):
   drawdown = GPMultiRateDrawdowntest.objects.get(id=id)   
   if request.method == 'POST' :
       drawdown.delete()
       drawdown.file_Name.delete()
       return redirect ('gpmultiratedrawdowntestanalysis:list_gpmultirate_test_data')
   return render (request, 'gpmultiratedrawdowntestanalysis/gpmultirate_test_data_confirm_delete.html', {'drawdown':drawdown})

def upload_Multi_Rate_test_data(request, id): 
    drawdown= GPMultiRateDrawdowntest.objects.get(id=id)     
    chart =Calculate_MultiRate_Test(request, id)   
    form = GPMultiRateDrawdownTestUploadForm(request.POST or None, instance=drawdown)     
    if request.method =="POST":  
        if drawdown.test_Type =='Constant_Rate':     
            ...
            form = GPMultiRateDrawdownTestUploadForm(request.POST, request.FILES, instance=drawdown)        
            if form.is_valid():
                form.save()  
                chart =Calculate_MultiRate_Test(request, id)        
                return render (request, 'gpmultiratedrawdowntestanalysis/gpmultirate_upload.html', {'form': form, 'chart':chart, 'drawdown':drawdown})
    return render (request, 'gpmultiratedrawdowntestanalysis/gpmultirate_upload.html', {'form': form, 'chart':chart, 'drawdown':drawdown})
