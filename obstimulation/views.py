...
from .forms import OBStimulationForm
from .utils import get_plot, get_plot1

# Create your views here.
def list_obstimulation_data(request):
    selectedwell = SelectedObserver.objects.first()   
    stim_datas = OBStimulation.objects.filter(obwellid =selectedwell.wellid).all()     
    ...
    chart = get_plot(x,y,y1,y2)  
    return render (request, 'obstimulation/obstimulation.html', {'stim_datas': stim_datas,'chart':chart, 'avgliqgain':avgliqgain,'avgoilgain':avgoilgain  })   

def create_obstimulation_data(request):    
   stim_data = OBStimulation()
   selectedwell = SelectedObserver.objects.first()  
   stim_data.obfgid = selectedwell.fgid
   stim_data.obwellid = selectedwell.wellid   
   form = OBStimulationForm(request.POST or None, instance=stim_data)
   if request.method =="POST":  
        form = OBStimulationForm(request.POST, request.FILES, instance=stim_data)       
        stim_data.fgid = selectedwell.fgid
        stim_data.wellid = selectedwell.wellid                       
        if form.is_valid(): 
            form.save()  
            return redirect ('obstimulation:list_obstimulation_data') 
   return render (request, 'obstimulation/obstimulation_form.html', {'form': form})

def update_obstimulation_data(request, id): 
   stim_data = OBStimulation.objects.get(id=id)  
   form = OBStimulationForm(request.POST or None, instance=stim_data) 
   if request.method =="POST":
        form = OBStimulationForm(request.POST, request.FILES, instance=stim_data)        
        if form.is_valid():
            form.save()           
            return redirect ('obstimulation:list_obstimulation_data')
   return render (request, 'obstimulation/obstimulation_form.html', {'form': form, 'stim_data':stim_data})

def delete_obstimulation_data(request, id):
   stim_data = OBStimulation.objects.get(id=id)   
   if request.method == 'POST' :
       stim_data.delete()
       return redirect ('obstimulation:list_obstimulation_data')
   return render (request, 'obstimulation/obstimulation_confirm_delete.html', {'stim_data':stim_data})


def detail_obstimulation_data(request, id): 
    oilgain =0
    selectedwell = SelectedObserver.objects.first()    
    stim_data = OBStimulation.objects.get(id=id)
   ...
   # for plot 
    ...
    form = OBStimulationForm(request.POST or None, instance=stim_data) 
    return render (request, 'obstimulation/obstimulation_data_detail.html', {'form': form, 'stim_data':stim_data, 
