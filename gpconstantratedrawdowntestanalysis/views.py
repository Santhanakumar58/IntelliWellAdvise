...
from gpconstantpressuredrawdowntestanalysis.utils import constant_Pressure_plot
from multiratedrawdowntestanalysis.utils import multiratetestplot
from selectedGasProducer.models import SelectedGasProducer
from .forms import GPConstantRateDrawdownTestForm, GPConstantRateDrawdownTestUploadForm
from .models import GPConstantRateDrawdowntestModel, filepath
import pandas as pd
import numpy as np
...
def list_gpconst_rate_drawdown_test_data(request):
    selectedwell = SelectedGasProducer.objects.first()   
    drawdown_test_datas = GPConstantRateDrawdowntestModel.objects.filter(gpwellid =selectedwell.wellid).all()   
    return render (request, 'gpconstantratedrawdowntestanalysis/gpconstantrate_test_data.html', {'drawdown_test_datas': drawdown_test_datas })   
 
def create_gpconst_rate_drawdown_test_data(request):    
    drawdown = GPConstantRateDrawdowntestModel()
    selectedwell = SelectedGasProducer.objects.first()  
    drawdown.fgid = selectedwell.fgid
    drawdown.wellid = selectedwell.wellid 
    form = GPConstantRateDrawdownTestForm(request.POST or None, instance=drawdown)
    if request.method =="POST": 
        drawdown.fgid = selectedwell.fgid
        drawdown.wellid = selectedwell.wellid  
        form = GPConstantRateDrawdownTestForm(request.POST, request.FILES, instance=drawdown)                    
        if form.is_valid(): 
            form.save()  
            file_Name = form.instance
            return redirect ('gpconstantratedrawdowntestanalysis:list_gpconst_rate_drawdown_test_data') 
    return render (request, 'gpconstantratedrawdowntestanalysis/gpconstantrate_test_data_form.html', {'form': form})

def update_gpconst_rate_drawdown_test_data(request, id): 
   drawdown= GPConstantRateDrawdowntestModel.objects.get(id=id)  
   form = GPConstantRateDrawdownTestForm(request.POST or None, instance=drawdown) 
   if request.method =="POST":       
        form = GPConstantRateDrawdownTestForm(request.POST, request.FILES, instance=drawdown)        
        if form.is_valid():
            form.save()           
            return redirect ('gpconstantratedrawdowntestanalysis:list_gpconst_rate_drawdown_test_data')
   return render (request, 'gpconstantratedrawdowntestanalysis/gpconstantrate_test_data_form.html', {'form': form, 'drawdown':drawdown})

def delete_gpconst_rate_drawdown_test_data(request, id):
   drawdown = GPConstantRateDrawdowntestModel.objects.get(id=id)   
   print(drawdown)
   if request.method == 'POST' :
       drawdown.delete()
       ...
       return redirect ('gpconstantratedrawdowntestanalysis:list_gpconst_rate_drawdown_test_data')
   return render (request, 'gpconstantratedrawdowntestanalysis/gpconstantrate_test_data_confirm_delete.html', {'drawdown':drawdown})

def upload_Conatant_Rate_test_data(request, id): 
    drawdown= GPConstantRateDrawdowntestModel.objects.get(id=id)      
    chart =Calculate_GPConstant_Rate_test_data(request, id)   
    form = GPConstantRateDrawdownTestUploadForm(request.POST or None, instance=drawdown)     
    if request.method =="POST": 
        drawdown.liquid_Rate = request.POST['liquid_Rate']
        drawdown.guess_Value =  request.POST['guess_Value']       
        form = GPConstantRateDrawdownTestUploadForm(request.POST, request.FILES, instance=drawdown)        
        if form.is_valid():
            form.save()  
            chart =Calculate_GPConstant_Rate_test_data(request, id)        
            return render (request, 'gpconstantratedrawdowntestanalysis/gpconstantrate_upload.html', {'form': form, 'chart':chart, 'drawdown':drawdown})
    return render (request, 'gpconstantratedrawdowntestanalysis/gpconstantrate_upload.html', {'form': form, 'chart':chart, 'drawdown':drawdown})
   

def upload_GPConstant_Pressure_test_data(request, id): 
    drawdown= GPConstantRateDrawdowntestModel.objects.get(id=id)     
    chart =Calculate_GPConstant_Pressure_Test(request, id)   
    form = GPConstantRateDrawdownTestUploadForm(request.POST or None, instance=drawdown)     
    if request.method =="POST":  
        if drawdown.test_Type =='GPConstant_Rate':     
            drawdown.liquid_Rate = request.POST['liquid_Rate']
            drawdown.guess_Value =  request.POST['guess_Value']       
            form = GPConstantRateDrawdownTestUploadForm(request.POST, request.FILES, instance=drawdown)        
            if form.is_valid():
                form.save()  
                chart =Calculate_GPConstant_Pressure_Test(request, id)        
                return render (request, 'gpdrawdowntestanalysis/gpdrawdown_test_data_upload.html', {'form': form, 'chart':chart, 'drawdown':drawdown})
    return render (request, 'gpdrawdowntestanalysis/gpdrawdown_test_data_upload.html', {'form': form, 'chart':chart, 'drawdown':drawdown})
...
def Calculate_GPConstant_Rate_test_data(request, id): 
    drawdown= GPConstantRateDrawdowntestModel.objects.get(id=id) 
...
def Calculate_GPConstant_Pressure_Test(request, id):
    drawdown= GPConstantRateDrawdowntestModel.objects.get(id=id) 
    path =drawdown.file_Name  
    filename = path.name
    path1 = 'C:/SanthanaKumar/PythonWellAdvisorNew/WellAdvisorPython/media/'
