...
from .forms import WISlicklineForm
from .utils import get_plot, get_plot1

# Create your views here.
def list_wislickline_data(request):
    selectedwell = SelectedWaterInjector.objects.first()   
    slick_datas = WISlickline.objects.filter(wellid =selectedwell.wellid).all()   
    print (slick_datas)  
    ...
    return render (request, 'wislickline/wislickline.html', {'slick_datas': slick_datas,'chart':chart, 'avgliqgain':avgliqgain,'avgoilgain':avgoilgain  })   

def create_wislickline_data(request):    
   slick_data = WISlickline()
   selectedwell = SelectedWaterInjector.objects.first()  
   slick_data.fgid = selectedwell.fgid
   slick_data.wellid = selectedwell.wellid   
   form = WISlicklineForm(request.POST or None, instance=slick_data)
   if request.method =="POST":  
        form = WISlicklineForm(request.POST, request.FILES, instance=slick_data)       
        slick_data.fgid = selectedwell.fgid
        slick_data.wellid = selectedwell.wellid                       
        if form.is_valid(): 
            form.save()  
            return redirect ('wislickline:list_wislickline_data') 
   return render (request, 'wislickline/wislickline_form.html', {'form': form})

def update_wislickline_data(request, id): 
   slick_data = WISlickline.objects.get(id=id)  
   form = WISlicklineForm(request.POST or None, instance=slick_data) 
   if request.method =="POST":
        form = WISlicklineForm(request.POST, request.FILES, instance=slick_data)        
        if form.is_valid():
            form.save()           
            return redirect ('wislickline:list_wislickline_data')
   return render (request, 'wislickline/wislickline_form.html', {'form': form, 'slick_data':slick_data})

def delete_wislickline_data(request, id):
   slick_data = WISlickline.objects.get(id=id)   
   if request.method == 'POST' :
       slick_data.delete()
       return redirect ('wislickline:list_wislickline_data')
   return render (request, 'wislickline/wislickline_confirm_delete.html', {'slick_data':slick_data})


def detail_wislickline_data(request, id): 
    oilgain =0
    selectedwell = SelectedWaterInjector.objects.first()    
    slick_data = WISlickline.objects.get(id=id)
   ...
   # for plot 
    ...
    form = WISlicklineForm(request.POST or None, instance=slick_data) 
    return render (request, 'wislickline/wislickline_data_detail.html', {'form': form, 'slick_data':slick_data, 
