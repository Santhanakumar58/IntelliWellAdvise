...
from .forms import WICoiltubingForm
from .utils import get_plot, get_plot1
from .models import WICoiltubing


# Create your views here.
def list_wicoil_tubing_data(request):
    selectedwell = SelectedWaterInjector.objects.first()   
    coil_tubing_datas = WICoiltubing.objects.filter(wellid =selectedwell.wellid).all()    
    ...
    return render (request, 'wicoiltubing/wicoil_tubing_data.html', {'coil_tubing_datas': coil_tubing_datas,'chart':chart, 'avgliqgain':avgliqgain,'avgoilgain':avgoilgain  })   
 
def create_wicoil_tubing_data(request):    
   coil_tubing_data = WICoiltubing()
   selectedwell = SelectedWaterInjector.objects.first()  
   coil_tubing_data.fgid = selectedwell.fgid
   coil_tubing_data.wellid = selectedwell.wellid   
   form = WICoiltubingForm(request.POST or None, instance=coil_tubing_data)
   if request.method =="POST":  
        form = WICoiltubingForm(request.POST, request.FILES, instance=coil_tubing_data)       
        coil_tubing_data.fgid = selectedwell.fgid
        coil_tubing_data.wellid = selectedwell.wellid                       
        if form.is_valid(): 
            form.save()  
            return redirect ('wicoiltubing:list_wicoil_tubing_data') 
   return render (request, 'wicoiltubing/wicoil_tubing_data_form.html', {'form': form})

def update_wicoil_tubing_data(request, id): 
   coil_tubing_data = WICoiltubing.objects.get(id=id)  
   form = WICoiltubingForm(request.POST or None, instance=coil_tubing_data) 
   if request.method =="POST":
        form = WICoiltubingForm(request.POST, request.FILES, instance=coil_tubing_data)        
        if form.is_valid():
            form.save()           
            return redirect ('wicoiltubing:list_wicoil_tubing_data')
   return render (request, 'wicoiltubing/wicoil_tubing_data_form.html', {'form': form, 'coil_tubing_data':coil_tubing_data})

def delete_wicoil_tubing_data(request, id):
   coil_tubing_data = WICoiltubing.objects.get(id=id)   
   if request.method == 'POST' :
       coil_tubing_data.delete()
       return redirect ('wicoiltubing:list_wicoil_tubing_data')
   return render (request, 'wicoiltubing/wicoil_tubing_data_confirm_delete.html', {'coil_tubing_data':coil_tubing_data})

def detail_wicoil_tubing_data(request, id): 
    oilgain =0
    selectedwell = SelectedWaterInjector.objects.first()    
    coil_tubing_data = WICoiltubing.objects.get(id=id)
   ...
   # for plot 
    ...
    form = WICoiltubingForm(request.POST or None, instance=coil_tubing_data) 
    return render (request, 'wicoiltubing/wicoil_tubing_data_detail.html', {'form': form, 'coil_tubing_data':coil_tubing_data, 
