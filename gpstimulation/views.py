...
from .forms import GPStimulationForm
from .utils import get_plot, get_plot1

# Create your views here.
def list_gpstimulation_data(request):
    selectedwell = SelectedGasProducer.objects.first()   
    stim_datas = GPStimulation.objects.filter(gpwellid =selectedwell.wellid).all()     
    ...
    chart = get_plot(x,y,y1,y2)  
    return render (request, 'gpstimulation/gpstimulation.html', {'stim_datas': stim_datas,'chart':chart, 'avgliqgain':avgliqgain,'avgoilgain':avgoilgain  })   

def create_gpstimulation_data(request):    
   stim_data = GPStimulation()
   selectedwell = SelectedGasProducer.objects.first()  
   stim_data.gpfgid = selectedwell.fgid
   stim_data.gpwellid = selectedwell.wellid   
   form = GPStimulationForm(request.POST or None, instance=stim_data)
   if request.method =="POST":  
        form = GPStimulationForm(request.POST, request.FILES, instance=stim_data)       
        stim_data.fgid = selectedwell.fgid
        stim_data.wellid = selectedwell.wellid                       
        if form.is_valid(): 
            form.save()  
            return redirect ('gpstimulation:list_gpstimulation_data') 
   return render (request, 'gpstimulation/gpstimulation_form.html', {'form': form})

def update_gpstimulation_data(request, id): 
   stim_data = GPStimulation.objects.get(id=id)  
   form = GPStimulationForm(request.POST or None, instance=stim_data) 
   if request.method =="POST":
        form = GPStimulationForm(request.POST, request.FILES, instance=stim_data)        
        if form.is_valid():
            form.save()           
            return redirect ('gpstimulation:list_gpstimulation_data')
   return render (request, 'gpstimulation/gpstimulation_form.html', {'form': form, 'stim_data':stim_data})

def delete_gpstimulation_data(request, id):
   stim_data = GPStimulation.objects.get(id=id)   
   if request.method == 'POST' :
       stim_data.delete()
       return redirect ('gpstimulation:list_gpstimulation_data')
   return render (request, 'gpstimulation/gpstimulation_confirm_delete.html', {'stim_data':stim_data})


def detail_gpstimulation_data(request, id): 
    oilgain =0
    selectedwell = SelectedGasProducer.objects.first()    
    stim_data = GPStimulation.objects.get(id=id)
   ...
   # for plot 
    ...
    form = GPStimulationForm(request.POST or None, instance=stim_data) 
    return render (request, 'gpstimulation/gpstimulation_data_detail.html', {'form': form, 'stim_data':stim_data, 
