...
def get_plot(t,p,m1,c1,m2,c2,s,k,re, your_guess):
    ...
    plt.axvspan(0, t[your_guess], color='orange', alpha=0.3)
    plt.axvspan(t[your_guess], max(t), color='green', alpha=0.3)

    plt.title('Constant Rate Drawdown Test Plot (BHFP vs Time)', size=14, pad=10)
  ...
  ## plot the separate MTR and LTR region
    ...
    plt.title('Semilog Plot of Constant Rate Drawdown Test(BHFP vs Time)', size=12, pad=10)
    plt.xlabel('Time (hours)', size=12); plt.ylabel('Pressure (psia)', size=12)
    plt.xlim(xmax=max(t))
...
def multiratetestplot(t,p, x,y, c, m, k,s, time_arr):    
    ...
    plt.subplot(1,2,1)
    plt.plot(time_arr, p, '.-', color='green')
    plt.xlim(0, max(t))
    plt.title('Multi-Rate Test Pressure Profile', size=14, pad=10)
    plt.xlabel(r'Time (hours)', size=12); plt.ylabel(r'Pressure (psi)', size=12)
    plt.grid(True, which='both', color='black', linewidth=0.1)

    plt.subplot(1,2,2)
    plt.plot(x, y, '.', color='green')
    plt.xlim(0, max(x))
    plt.title('Drawdown Plot for Multi-Rate Flow', size=14, pad=10)
    plt.xlabel(r'$F_p$', size=12); plt.ylabel(r'$\frac{p_i-p_{wf}}{q_n}$ (psi-D/STB)', size=12)
...
def constant_Pressure_plot(t, q, pwf, pi, Bo, mu_oil, h, poro, ct, rw, k,s,m,c):
    ...
    plt.title('Constant Pressure Test Rate vs Time', size=12, pad=10)
    ...
    plt.title('Semilog Plot of Reciprocal Rate vs Time', size=12, pad=10)
