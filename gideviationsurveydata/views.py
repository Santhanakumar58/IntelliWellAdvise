...
from .forms import GIDeviationSurveyDataForm
from selectedGasInjector.models import SelectedGasInjector
from gidrillingwelldata.models import GIDrillingWellData
from gideviationsurveydata.models import GIDeviationsurveydata
import pandas as pd
import os
from django.core.files.storage import FileSystemStorage

def list_gideviationsurvey(request):
    selectedwell= SelectedGasInjector.objects.all().first()
    gideviationsurveys = GIDeviationsurveydata.objects.filter(giwellid = selectedwell.wellid) 
    gidrillingwelldata = GIDrillingWellData.objects.filter(giwellid=selectedwell.wellid).first()
    sortedmodels = gideviationsurveys.order_by('gimeasuredDepth')   
    x=[x.northSouth for x in sortedmodels]
    y=[y.eastWest for y in sortedmodels]
    z=[-z.tvd for z in sortedmodels]
    chart = get_plot(x,y,z)
    return render (request, 'gideviationsurveydata/gideviationsurvey.html', {'gideviationsurveys': sortedmodels,'chart':chart })

def create_gideviationsurvey(request):
    selctedoilproducer = SelectedGasInjector.objects.all().first()
    previousmodel = GIDeviationsurveydata.objects.all().last()
    gideviationsurvey = GIDeviationsurveydata()   
    gideviationsurvey.fgId = selctedoilproducer.fgid
    gideviationsurvey.wellid= selctedoilproducer.wellid
    form = GIDeviationSurveyDataForm(request.POST or None, instance=gideviationsurvey) 
    if request.method == "POST":       
        curdepth = float(request.POST["measuredDepth"])
        curangle = float(request.POST["angle"])
        curazimuth = float(request.POST["azimuth"])
        premodel =GIDeviationsurveydata()
        gideviationsurveys = GIDeviationsurveydata.objects.filter(wellid =selctedoilproducer.wellid)
        if float(curdepth) > previousmodel.measuredDepth:
           premodel = previousmodel
        else:
            for data in gideviationsurveys:
        ...
        gideviationsurvey.northSouth = north + premodel.northSouth
        east = calculate_East(prevdepth, prevangle, prevazimuth,curdepth, curangle, curazimuth)
        gideviationsurvey.eastWest = east + premodel.eastWest
        vert = calculate_Vert(prevdepth, prevangle, prevazimuth,curdepth, curangle, curazimuth)
        gideviationsurvey.tvd = vert + premodel.tvd
        gideviationsurvey.dogLeg = calculate_Dogleg(prevangle, prevazimuth, curangle, curazimuth)
        netdrift = calculate_Net_Drift(prevdepth, prevangle, curdepth, curangle)
        gideviationsurvey.netDrift = netdrift + premodel.netDrift
        vertsection = calculate_Vertical_Section(netdrift,  prevazimuth, curazimuth)
        gideviationsurvey.verticalSection = vertsection + premodel.verticalSection
        netdir = calculate_Net_Direction(prevdepth, curdepth, curangle, curazimuth)  
        gideviationsurvey.netDirection = netdir + premodel.netDirection 
        form = GIDeviationSurveyDataForm(request.POST or None, instance=gideviationsurvey) 
        if form.is_valid():
            form.save()
            return redirect ('gideviationsurveydata:list_gideviationsurvey')
    return render (request, 'gideviationsurveydata/gideviationsurvey_form.html', {'form': form})

def update_gideviationsurvey(request, id):
    selctedoilproducer = SelectedGasInjector.objects.all().first()
    gideviationsurvey= GIDeviationsurveydata.objects.get(id=id)
    form = GIDeviationSurveyDataForm(request.POST or None, instance=gideviationsurvey) 
    gideviationsurveys = GIDeviationsurveydata.objects.filter(wellid =selctedoilproducer.wellid)
    sortedmodels = gideviationsurveys.order_by('measuredDepth') 
    ...
    if request.method == "POST":       
        ...
        if form.is_valid():
            form.save()  
        return redirect ('gideviationsurveydata:list_gideviationsurvey')
    return render (request, 'gideviationsurveydata/gideviationsurvey_form.html', {'form': form, 'gideviationsurvey':gideviationsurvey})

def delete_gideviationsurvey(request, id):
   gideviationsurvey = GIDeviationsurveydata.objects.get(id=id)
   
   if request.method == 'POST' :
       gideviationsurvey.delete()
       return redirect ('gideviationsurveydata:list_gideviationsurvey')
   return render (request, 'gideviationsurveydata/gideviationsurvey_confirm_delete.html', {'gideviationsurvey':gideviationsurvey})

def Import_Deviation_SurveyData(request):     
    if request.method == 'POST' and request.FILES['myfile']:        
        selectedwell = SelectedGasInjector.objects.first()   
        id = selectedwell.wellid
        devdata = GIDeviationsurveydata.objects.filter(wellid = id)
        devdata.delete()
        ...
        for dbframe in dbframe.itertuples():
            if dbframe.measureddepth ==0:
                obj = GIDeviationsurveydata.objects.create(fgId =selectedwell.fgid, wellid= id, measuredDepth=dbframe.measureddepth,angle=dbframe.angle, azimuth=dbframe.azimuth,
                ...
                if curangle==0 | curazimuth == 0:
                    obj = GIDeviationsurveydata.objects.create(fgId =selectedwell.fgid, wellid= id, measuredDepth=curdepth,angle=curangle, azimuth=curazimuth,
        ...
        return render(request, 'gideviationsurveydata/Import_excel.html', {'uploaded_file_url': uploaded_file_url, 'devdata':devdata }) 
    
    return render(request, 'gideviationsurveydata/Import_excel.html',{})
