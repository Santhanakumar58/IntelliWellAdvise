...
from .forms import GPCompletionForm
...
from gpcasings.models import GPCasingModel, GPCasingSizeModel
import numpy as np
import pandas as pd

# Create your views here.
def list_gpcompletion_data(request):
    selectedwell = SelectedGasProducer.objects.all().first()         
    completion_datas = GPWellcompletion.objects.filter(gpwellid =selectedwell.wellid).all().order_by('gpequip_Md')  
    casings = GPCasingModel.objects.filter(gpwellid=selectedwell.wellid).all().order_by('gpshoedepth') 
    ...
    for casing in casings:    
        csize = GPCasingSizeModel.objects.get(gpcasingSize = casing.casingSize)  
    ...
    deviationdata = GPDeviationsurveydata.objects.filter(gpwellid=selectedwell.wellid).all()
    equipmentDepth=[]
    equipmentName=[]
    equipmentx =[]
    east=0
    ...
    return render (request, 'gpwellcompletion/gpcompletion_data.html', {'completion_datas': completion_datas, 'chart2':chart2})   
 
def create_gpcompletion_data(request):    
   completion_data = GPWellcompletion() 
   selectedwell = SelectedGasProducer.objects.all().first()     
   completion_data.gpfgid = selectedwell.fgid
   completion_data.gpwellid = selectedwell.wellid   
   form = GPCompletionForm(request.POST or None, instance=completion_data)
   if request.method =="POST":  
        form = GPCompletionForm(request.POST, request.FILES, instance=completion_data)       
        completion_data.gpfgid = selectedwell.fgid
        completion_data.gpwellid = selectedwell.wellid    
        md =  float(request.POST['equip_Md'])       
        deviation_datas =   GPDeviationsurveydata.objects.filter(gpwellid = selectedwell.wellid).all()        
        tvd = tvdCalculation(deviation_datas, md)   
        completion_data.equip_Tvd = round(tvd[0],2) 
        completion_data.equip_Angle = round(tvd[1],2)             
        if form.is_valid(): 
            form.save()  
            return redirect ('gpwellcompletion:list_completion_data') 
   return render (request, 'gpwellcompletion/completion_data_form.html', {'form': form})

def update_gpcompletion_data(request, id): 
   completion_data = GPWellcompletion.objects.get(id=id)  
   form = GPCompletionForm(request.POST or None, instance=completion_data) 
   if request.method =="POST":
        form = GPCompletionForm(request.POST, request.FILES, instance=completion_data)  
        md =  float(request.POST['equip_Md'])   
        deviation_datas =   GPDeviationsurveydata.objects.filter(wellid = completion_data.wellid).all()        
        tvd = tvdCalculation(deviation_datas, md)   
        completion_data.equip_Tvd = round(tvd[0],2) 
        completion_data.equip_Angle = round(tvd[1],2)                
        if form.is_valid():
            form.save()           
            return redirect ('gpwellcompletion:list_completion_data')
   return render (request, 'gpwellcompletion/completion_data_form.html', {'form': form, 'completion_data':completion_data})

def delete_gpcompletion_data(request, id):
   completion_data = GPWellcompletion.objects.get(id=id)   
   if request.method == 'POST' :
       completion_data.delete()
       return redirect ('gpwellcompletion:list_completion_data')
   return render (request, 'gpwellcompletion/completion_data_confirm_delete.html', {'completion_data':completion_data})
