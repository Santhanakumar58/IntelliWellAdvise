...
from .forms import GPSlicklineForm
from .utils import get_plot, get_plot1

# Create your views here.
def list_gpslickline_data(request):
    selectedwell = SelectedGasProducer.objects.first()   
    slick_datas = GPSlickline.objects.filter(gpwellid =selectedwell.wellid).all()   
    print (slick_datas)  
    ...
    return render (request, 'gpslickline/gpslickline.html', {'slick_datas': slick_datas,'chart':chart, 'avgliqgain':avgliqgain,'avgoilgain':avgoilgain  })   

def create_gpslickline_data(request):    
   slick_data = GPSlickline()
   selectedwell = SelectedGasProducer.objects.first()  
   slick_data.fgid = selectedwell.fgid
   slick_data.wellid = selectedwell.wellid   
   form = GPSlicklineForm(request.POST or None, instance=slick_data)
   if request.method =="POST":  
        form = GPSlicklineForm(request.POST, request.FILES, instance=slick_data)       
        slick_data.fgid = selectedwell.fgid
        slick_data.wellid = selectedwell.wellid                       
        if form.is_valid(): 
            form.save()  
            return redirect ('gpslickline:list_gpslickline_data') 
   return render (request, 'gpslickline/gpslickline_form.html', {'form': form})

def update_gpslickline_data(request, id): 
   slick_data = GPSlickline.objects.get(id=id)  
   form = GPSlicklineForm(request.POST or None, instance=slick_data) 
   if request.method =="POST":
        form = GPSlicklineForm(request.POST, request.FILES, instance=slick_data)        
        if form.is_valid():
            form.save()           
            return redirect ('gpslickline:list_gpslickline_data')
   return render (request, 'gpslickline/gpslickline_form.html', {'form': form, 'slick_data':slick_data})

def delete_gpslickline_data(request, id):
   slick_data = GPSlickline.objects.get(id=id)   
   if request.method == 'POST' :
       slick_data.delete()
       return redirect ('gpslickline:list_gpslickline_data')
   return render (request, 'gpslickline/gpslickline_confirm_delete.html', {'slick_data':slick_data})


def detail_gpslickline_data(request, id): 
    oilgain =0
    selectedwell = SelectedGasProducer.objects.first()    
    slick_data = GPSlickline.objects.get(id=id)
   ...
   # for plot 
    ...
    form = GPSlicklineForm(request.POST or None, instance=slick_data) 
    return render (request, 'gpslickline/gpslickline_data_detail.html', {'form': form, 'slick_data':slick_data, 
