...
from .forms import ConstantRateDrawdownTestForm, ConstantRateDrawdownTestUploadForm
from .models import ConstantRateDrawdowntestModel, filepath
import pandas as pd
import numpy as np
...
def list_const_rate_drawdown_test_data(request):
    selectedwell = SelectedOilProducer.objects.first()   
    drawdown_test_datas = ConstantRateDrawdowntestModel.objects.filter(wellid =selectedwell.wellid).all()   
    return render (request, 'constantratedrawdowntestanalysis/constantrate_test_data.html', {'drawdown_test_datas': drawdown_test_datas })   
 
def create_const_rate_drawdown_test_data(request):    
    drawdown = ConstantRateDrawdowntestModel()
    selectedwell = SelectedOilProducer.objects.first()  
    drawdown.fgid = selectedwell.fgid
    drawdown.wellid = selectedwell.wellid 
    form = ConstantRateDrawdownTestForm(request.POST or None, instance=drawdown)
    if request.method =="POST": 
        drawdown.fgid = selectedwell.fgid
        drawdown.wellid = selectedwell.wellid  
        form = ConstantRateDrawdownTestForm(request.POST, request.FILES, instance=drawdown)                    
        if form.is_valid(): 
            form.save()  
            file_Name = form.instance
            return redirect ('constantratedrawdowntestanalysis:list_const_rate_drawdown_test_data') 
    return render (request, 'constantratedrawdowntestanalysis/constantrate_test_data_form.html', {'form': form})

def update_const_rate_drawdown_test_data(request, id): 
   drawdown= ConstantRateDrawdowntestModel.objects.get(id=id)  
   form = ConstantRateDrawdownTestForm(request.POST or None, instance=drawdown) 
   if request.method =="POST":       
        form = ConstantRateDrawdownTestForm(request.POST, request.FILES, instance=drawdown)        
        if form.is_valid():
            form.save()           
            return redirect ('constantratedrawdowntestanalysis:list_const_rate_drawdown_test_data')
   return render (request, 'constantratedrawdowntestanalysis/constantrate_test_data_form.html', {'form': form, 'drawdown':drawdown})

def delete_const_rate_drawdown_test_data(request, id):
   drawdown = ConstantRateDrawdowntestModel.objects.get(id=id)   
   print(drawdown)
   if request.method == 'POST' :
       drawdown.delete()
       ...
       return redirect ('constantratedrawdowntestanalysis:list_const_rate_drawdown_test_data')
   return render (request, 'constantratedrawdowntestanalysis/constantrate_test_data_confirm_delete.html', {'drawdown':drawdown})
...
def upload_Conatant_Rate_test_data(request, id): 
    ...
    if request.method =="POST": 
        ...
        if form.is_valid():
            form.save()  
            chart =Calculate_Constant_Rate_test_data(request, id)        
            return render (request, 'constantratedrawdowntestanalysis/constantrate_upload.html', {'form': form, 'chart':chart, 'drawdown':drawdown})
    return render (request, 'constantratedrawdowntestanalysis/constantrate_upload.html', {'form': form, 'chart':chart, 'drawdown':drawdown})
