...
def get_plot(x,y,z,xa, ya, x1,y1,z1, x2,y2,z2,xb,yb, x3,y3,z3, x4,y4,z4, xc, yc, x5,y5,z5, gpcasinga, gpcasingb, gpcasingc):
    plt.switch_backend('AGG')
    plt.figure(figsize=(6,10))
    plt.title("GPCasing Diagram")
    plt.plot(x,y, color='red')
    plt.plot(z,y, color='red')   
    labela =f"{ya}"
    plt.annotate(gpcasinga + "gpcasing at - " + labela, (xa,ya), textcoords='offset points', xytext=(0,-15), ha='center')
    plt.plot(x1,y1, color='green')
    plt.plot(z1,y1, color='green')
    plt.plot(x2,y2, color='red')
    plt.plot(z2,y2, color='red')
    labelb =f"{yb}"
    plt.annotate(gpcasingb + "gpcasing at - " + labelb, (xb,yb), textcoords='offset points', xytext=(0,-15), ha='center')
    plt.plot(x3,y3, color='green')
    plt.plot(z3,y3, color='green')
    plt.plot(x4,y4, color='red')
    plt.plot(z4,y4, color='red')
    labelc =f"{yc}"
    plt.annotate(gpcasingc + "gpcasing at - " + labelc, (xc,yc), textcoords='offset points', xytext=(0,-15), ha='center')
...
def get_plot2(widths, depths, hangers, cements, deviationdata, gpcasingdefs):
    plt.switch_backend('AGG')
    ...
    for i in range(len(widths)):
        ...
        plt.plot(x,y, color='red')
        labela =f"{depths[i]}"
        plt.annotate(gpcasingdefs[i] + "  gpcasing at - " + labela, (xa,-depths[i]), textcoords='offset points', xytext=(0,-15), ha='center', color="blue")
...
def get_dummyplot():
    plt.switch_backend('AGG')
    plt.figure(figsize=(6,10))
    plt.title("Gas Producer Casing Diagram")
    x=[100,100]
    y=[0,100]
    z=[200,200]
    ya="Outer Casing"
