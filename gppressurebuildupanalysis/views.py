...
from .forms import GPPressureBuildupDataUploadForm, GPBuildupTestForm, GPBuildupTestUploadForm
from django.contrib import messages
import pandas as pd
import numpy as np
from . utils import get_constabt_Rate_Buildup_plot, get_limits, get_totalplot, get_totalplot1
...
def list_gpbuildup_test_data(request):
    selectedwell = SelectedGasProducer.objects.first()   
    buildup_test_datas = GPPressureBuildupModel.objects.filter(gpwellid =selectedwell.wellid).all()   
    return render (request, 'gppressurebuildupanalysis/gppressurebuildupanalysis.html', {'buildup_test_datas': buildup_test_datas })   
 
def create_gpbuildup_test_data(request):    
    buildup = GPPressureBuildupModel()
    selectedwell = SelectedGasProducer.objects.first()  
    buildup.fgid = selectedwell.fgid
    buildup.wellid = selectedwell.wellid   
    form = GPBuildupTestForm(request.POST or None, instance=buildup)
    if request.method =="POST": 
        buildup.fgid = selectedwell.fgid
        buildup.wellid = selectedwell.wellid  
        form = GPBuildupTestForm(request.POST, request.FILES, instance=buildup)                    
        if form.is_valid(): 
            form.save()  
...
def update_gpbuildup_test_data(request, id): 
   buildup= GPPressureBuildupModel.objects.get(id=id)  
   form = GPBuildupTestForm(request.POST or None, instance=buildup) 
   if request.method =="POST":       
        form = GPBuildupTestForm(request.POST, request.FILES, instance=buildup)        
        if form.is_valid():
            form.save()           
            return redirect ('gppressurebuildupanalysis:list_gpbuildup_test_data')
   return render (request, 'gppressurebuildupanalysis/gppressurebuildupanalysis_form.html', {'form': form, 'buildup':buildup})

def delete_gpbuildup_test_data(request, id):
   buildup = GPPressureBuildupModel.objects.get(id=id)   
   if request.method == 'POST' :
       buildup.delete()
       buildup.dataFile.delete()
       return redirect ('gppressurebuildupanalysis:list_gpbuildup_test_data')
   return render (request, 'gppressurebuildupanalysis/gppressurebuildupanalysis_confirm_delete.html', {'buildup':buildup})

def upload_buildup_test_data(request, id): 
    buildup= GPPressureBuildupModel.objects.get(id=id) 
    if buildup.test_Type =='Constant_Rate':
        chart =Calculate_Constant_Rate_Buildup_test(id)   
        form =GPBuildupTestUploadForm(request.POST or None, instance=buildup)     
        if request.method =="POST":  
            if buildup.test_Type =='Constant_Rate':     
                buildup.oil_Prod_Rate = request.POST['oil_Prod_Rate']
                buildup.guess_Value =  request.POST['guess_Value']       
                form = GPBuildupTestUploadForm(request.POST, request.FILES, instance=buildup)        
                if form.is_valid():
                    form.save()  
                    chart =Calculate_Constant_Rate_Buildup_test( id)        
                    return render (request, 'gppressurebuildupanalysis/gpbuildup_test_data_upload.html', {'form': form, 'chart':chart, 'buildup':buildup})
        return render (request, 'gppressurebuildupanalysis/gpbuildup_test_data_upload.html', {'form': form, 'chart':chart, 'buildup':buildup})
    elif buildup.test_Type =='Constant_Pressure':
        chart =Calculate_Constant_Rate_Buildup_test(id)   
        form = GPPressureBuildupDataUploadForm(request.POST or None, instance=buildup)  
        return render (request, 'gppressurebuildupanalysis/gpbuildup_test_data_upload.html', {'chart':chart})
...
def pbu_Analysis(request, id):
    ...
    if request.method == 'POST' :       
        ...
        return redirect ('gppressurebuildupanalysis:list_gpbuildup_test_data')
    return render (request, 'gppressurebuildupanalysis/gppressurebuildup_Analysis.html', {'chart':chart})
