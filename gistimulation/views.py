...
from .forms import GIStimulationForm
from .utils import get_plot, get_plot1

# Create your views here.
def list_gistimulation_data(request):
    selectedwell = SelectedGasInjector.objects.first()   
    stim_datas = GIStimulation.objects.filter(giwellid =selectedwell.wellid).all()     
    ...
    chart = get_plot(x,y,y1,y2)  
    return render (request, 'gistimulation/gistimulation.html', {'stim_datas': stim_datas,'chart':chart, 'avgliqgain':avgliqgain,'avgoilgain':avgoilgain  })   

def create_gistimulation_data(request):    
   stim_data = GIStimulation()
   selectedwell = SelectedGasInjector.objects.first()  
   stim_data.gifgid = selectedwell.fgid
   stim_data.giwellid = selectedwell.wellid   
   form = GIStimulationForm(request.POST or None, instance=stim_data)
   if request.method =="POST":  
        form = GIStimulationForm(request.POST, request.FILES, instance=stim_data)       
        stim_data.fgid = selectedwell.fgid
        stim_data.wellid = selectedwell.wellid                       
        if form.is_valid(): 
            form.save()  
            return redirect ('gistimulation:list_gistimulation_data') 
   return render (request, 'gistimulation/gistimulation_form.html', {'form': form})

def update_gistimulation_data(request, id): 
   stim_data = GIStimulation.objects.get(id=id)  
   form = GIStimulationForm(request.POST or None, instance=stim_data) 
   if request.method =="POST":
        form = GIStimulationForm(request.POST, request.FILES, instance=stim_data)        
        if form.is_valid():
            form.save()           
            return redirect ('gistimulation:list_gistimulation_data')
   return render (request, 'gistimulation/gistimulation_form.html', {'form': form, 'stim_data':stim_data})

def delete_gistimulation_data(request, id):
   stim_data = GIStimulation.objects.get(id=id)   
   if request.method == 'POST' :
       stim_data.delete()
       return redirect ('gistimulation:list_gistimulation_data')
   return render (request, 'gistimulation/gistimulation_confirm_delete.html', {'stim_data':stim_data})


def detail_gistimulation_data(request, id): 
    oilgain =0
    selectedwell = SelectedGasInjector.objects.first()    
    stim_data = GIStimulation.objects.get(id=id)
   ...
   # for plot 
    ...
    form = GIStimulationForm(request.POST or None, instance=stim_data) 
    return render (request, 'gistimulation/gistimulation_data_detail.html', {'form': form, 'stim_data':stim_data, 
