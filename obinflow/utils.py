...
from obinflow.models import OBDarcyModel, OBMultirateModel, OBProductivityIndexModel, OBStandingsModel, OBVogelModel, OBWigginsModel
...
def draw_CompositePR_PI (pimodels = OBProductivityIndexModel()):
    x=[] 
    y=[]
    x1=[] 
    y1=[]
...
def draw_compositeIPR_Vogel(vogelmodels = OBVogelModel()):
    x=[] 
    y=[]
    x1=[] 
    y1=[]
...
def draw_compositeIPR_Standing(standingmodels = OBStandingsModel()):
    x=[] 
    y=[]
    x1=[] 
    y1=[]
...
def draw_compositeIPR_Wiggins(wigginmodels= OBWigginsModel()):
    x=[] 
    y=[]
    x1=[] 
    y1=[]
...
def draw_CompositeIPR_Multirate(multiratemodels= OBMultirateModel()):
    x=[] 
    y=[]
    x1=[] 
    y1=[]
...
def draw_CompositeIPR_Darcy(darcymodels = OBDarcyModel()):
    x=[] 
    y=[]
    x1=[] 
    y1=[]
...
def draw_LayerIPR_Darcy(darcymodel = OBDarcyModel()):
    x=[] 
    y=[]  
...
def draw_LayerIPR_Multirate(multiratemodel = OBMultirateModel()):
    x=[] 
    y=[]
    logx=[]
    logy=[]
...
def draw_LayerIPR_Wiggin(wigginmodel = OBWigginsModel()):
    x=[] 
    y=[]
    pvt = BlackoilPVT.objects.filter(wellName =wigginmodel.pvt_Well, subLayer =wigginmodel.layer_Name).last()
    oilgravity = pvt.oilAPIgravity           
...
def draw_LayerIPR_Standing(standingmodel=OBStandingsModel()):
    x=[] 
    y=[]
    x1=[] 
    y1=[]
...
def draw_LayerIPR_Vogel(vogelmodel=OBVogelModel()):
    x=[] 
    y=[]
    pvt = BlackoilPVT.objects.filter(wellName =vogelmodel.pvt_Well, subLayer =vogelmodel.layer_Name).last()
    temperature = pvt.reservoirTemperature
...
def draw_LayerIPR_PI(pimodel=OBProductivityIndexModel()):
    x=[] 
    y=[] 
...
def draw_Multirateloglogplot(multiratemodel=OBMultirateModel()):
    logx=[]
    logy=[]
    resPres = multiratemodel.current_Reservoir_Pressure
    pwf1 = (math.pow(resPres, 2) - math.pow(multiratemodel.test_Pressure1, 2))
