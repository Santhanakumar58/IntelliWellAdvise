...
from .forms import CoiltubingOpsForm
from .utils import get_pieplot
from django.contrib import messages

def list_coil_tubing_ops_data(request, ctid): 
    print(ctid)
    selectedwell = SelectedOilProducer.objects.all().first()
    coil_tubing_ops_datas = CoiltubingOperation.objects.filter(wellid = selectedwell.wellid, coiltubingid =ctid).all()  
    ...
    return render (request, 'coiltubingoperations/coil_tubing_ops_data.html', {'coil_tubing_ops_datas': coil_tubing_ops_datas,'chart':chart, 'ctid':ctid})   
 
def create_coil_tubing_ops_data(request, ctid):
    coil_tubing_data = Coiltubing.objects.get(id=ctid)
    coil_tubing_ops_data = CoiltubingOperation()   
    ...
    form = CoiltubingOpsForm(request.POST or None, instance=coil_tubing_ops_data)
    if request.method =="POST":  
         form = CoiltubingOpsForm(request.POST, request.FILES, instance=coil_tubing_ops_data)       
         coil_tubing_ops_data.fgid = coil_tubing_data.fgid
         coil_tubing_ops_data.wellid = coil_tubing_data.wellid   
         coil_tubing_ops_data.coiltubingid =coil_tubing_data 
         ...
         if dat1 <coil_tubing_data.start_Date or dat1 >coil_tubing_data.end_Date:
            ...
            print(message)
            return render (request, 'coiltubingoperations/coil_tubing_ops_data_form.html', {'form': form, 'message':message, 'ctid':ctid})   
         ...
         if form.is_valid():
            form.save()  
            return redirect ('coiltubingoperations:list_coil_tubing_ops_data', ctid) 
    return render (request, 'coiltubingoperations/coil_tubing_ops_data_form.html', {'form': form,   'ctid':ctid})

def update_coil_tubing_ops_data(request, id):  
    coil_tubing_ops_data = CoiltubingOperation.objects.get(id=id) 
    ...
    if request.method =="POST":
        ...
        if (dat < dat1 or dat >dat2):
            ...
            return render (request, 'coiltubingoperations/coil_tubing_ops_data_update_form.html', {'form': form, 'coil_tubing_ops_data':coil_tubing_ops_data, 'id':id})
        timefrom = request.POST['time_from']
        timeto = request.POST['time_to']  
        t1 = pd.to_datetime(timefrom)
        t2 = pd.to_datetime(timeto)        
...
def delete_coil_tubing_ops_data(request, id):
    coil_tubing_ops_data = CoiltubingOperation.objects.get(id=id)  
    ctid =(coil_tubing_ops_data.coiltubingid).pk   
    print(ctid)
    if request.method == 'POST' :
        ...
        ctid =(coil_tubing_ops_data.coiltubingid).pk
        return redirect ('coiltubingoperations:list_coil_tubing_ops_data', ctid)
    return render (request, 'coiltubingoperations/coil_tubing_ops_data_confirm_delete.html', {'coil_tubing_ops_data':coil_tubing_ops_data, 'id':id})
