...
from .forms import OBSlicklineForm
from .utils import get_plot, get_plot1

# Create your views here.
def list_obslickline_data(request):
    selectedwell = SelectedObserver.objects.first()   
    slick_datas = OBSlickline.objects.filter(obwellid =selectedwell.wellid).all()   
    print (slick_datas)  
    ...
    return render (request, 'obslickline/obslickline.html', {'slick_datas': slick_datas,'chart':chart, 'avgliqgain':avgliqgain,'avgoilgain':avgoilgain  })   

def create_obslickline_data(request):    
   slick_data = OBSlickline()
   selectedwell = SelectedObserver.objects.first()  
   slick_data.fgid = selectedwell.fgid
   slick_data.wellid = selectedwell.wellid   
   form = OBSlicklineForm(request.POST or None, instance=slick_data)
   if request.method =="POST":  
        form = OBSlicklineForm(request.POST, request.FILES, instance=slick_data)       
        slick_data.fgid = selectedwell.fgid
        slick_data.wellid = selectedwell.wellid                       
        if form.is_valid(): 
            form.save()  
            return redirect ('obslickline:list_obslickline_data') 
   return render (request, 'obslickline/obslickline_form.html', {'form': form})

def update_obslickline_data(request, id): 
   slick_data = OBSlickline.objects.get(id=id)  
   form = OBSlicklineForm(request.POST or None, instance=slick_data) 
   if request.method =="POST":
        form = OBSlicklineForm(request.POST, request.FILES, instance=slick_data)        
        if form.is_valid():
            form.save()           
            return redirect ('obslickline:list_obslickline_data')
   return render (request, 'obslickline/obslickline_form.html', {'form': form, 'slick_data':slick_data})

def delete_obslickline_data(request, id):
   slick_data = OBSlickline.objects.get(id=id)   
   if request.method == 'POST' :
       slick_data.delete()
       return redirect ('obslickline:list_obslickline_data')
   return render (request, 'obslickline/obslickline_confirm_delete.html', {'slick_data':slick_data})


def detail_obslickline_data(request, id): 
    oilgain =0
    selectedwell = SelectedObserver.objects.first()    
    slick_data = OBSlickline.objects.get(id=id)
   ...
   # for plot 
    ...
    form = OBSlicklineForm(request.POST or None, instance=slick_data) 
    return render (request, 'obslickline/obslickline_data_detail.html', {'form': form, 'slick_data':slick_data, 
