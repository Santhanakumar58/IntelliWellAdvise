...
from .forms import WIRigworkoverForm
from wirigworkover.utils import get_plot, get_plot1
import datetime
import pandas as pd
import numpy as np
...
def list_wirig_workover_data(request):  
    selectedwell = SelectedWaterInjector.objects.first()    
    rig_workover_datas = WIRigworkover.objects.filter(wiwellid =selectedwell.wellid).all() 
    # for plot     
    rig_workover_datas = WIRigworkover.objects.filter(wiwellid =selectedwell.wellid).all()  
    ...
    return render (request, 'wirigworkover/wirig_wor_data.html', {'rig_workover_datas': rig_workover_datas, 'chart':chart, 'avgliqgain':avgliqgain, 'avgoilgain':avgoilgain })   

def create_wirig_workover_data(request):  
    selectedwell = SelectedWaterInjector.objects.first()    
    rig_workover_data = WIRigworkover()  
    rig_workover_data.fgid = selectedwell.fgid
    rig_workover_data.wellid = selectedwell.wellid   
    form = WIRigworkoverForm(request.POST or None, instance=rig_workover_data)
    if request.method =="POST":  
        form = WIRigworkoverForm(request.POST, request.FILES, instance=rig_workover_data)       
        rig_workover_data.fgid = selectedwell.fgid
        rig_workover_data.wellid = selectedwell.wellid                       
        if form.is_valid(): 
            form.save()  
            return redirect ('wirigworkover:list_wirig_workover_data') 
    return render (request, 'wirigworkover/wirig_wor_data_form.html', {'form': form})

def update_wirig_workover_data(request, id): 
    rig_workover_data = WIRigworkover.objects.get(id=id)  
    form = WIRigworkoverForm(request.POST or None, instance=rig_workover_data) 
    if request.method =="POST":
        form = WIRigworkoverForm(request.POST, request.FILES, instance=rig_workover_data)        
        if form.is_valid():
            form.save()           
            return redirect ('wirigworkover:list_wirig_workover_data')
    return render (request, 'wirigworkover/wirig_wor_data_form.html', {'form': form, 'rig_workover_data':rig_workover_data})

def delete_wirig_workover_data(request, id):
    rig_workover_data = WIRigworkover.objects.get(id=id)   
    if request.method == 'POST' :
        rig_workover_data.delete()
        return redirect ('wirigworkover:list_wirig_workover_data')
    return render (request, 'wirigworkover/wirig_wor_data_confirm_delete.html', {'rig_workover_data':rig_workover_data})

def detail_wirig_workover_data(request, id): 
    oilgain =0
    selectedwell = SelectedWaterInjector.objects.first()    
    rig_workover_data = WIRigworkover.objects.get(id=id)
   ...
   # for plot 
    ...
    form = WIRigworkoverForm(request.POST or None, instance=rig_workover_data) 
    return render (request, 'wirigworkover/wirig_wor_data_detail.html', {'form': form, 'rig_workover_data':rig_workover_data, 
...
def wiintervention_home(request):
   
   return render (request, 'wirigworkover/wiintervention_home.html', {})
